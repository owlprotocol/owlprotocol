...
import {IERC721DnaMintable} from './IERC721DnaMintable.sol';

/**
 * @dev ERC721DNA
 */
abstract contract ERC721DnaBaseMintable is ERC721Base, IERC721DnaMintable {
    ...
    function __ERC721DnaBase_init_unchained(address _minterRole, address _dnaRole) internal {
        ...
        if (AddressUpgradeable.isContract(ERC1820_REGISTRY)) {
            registry.updateERC165Cache(address(this), type(IERC721DnaMintable).interfaceId);
            registry.setInterfaceImplementer(address(this), type(IERC721DnaMintable).interfaceId | ONE, address(this));
        }
     ...
     */
    function mintWithDna(address to, uint256 tokenId, bytes memory dna) external onlyRole(MINTER_ROLE) returns (uint256) {
     ...
     * inheritdoc IERC721Dna
     */
    function safeMintWithDna(address to, uint256 tokenId, bytes memory dna) external onlyRole(MINTER_ROLE) returns (uint256) {
        inherentDna[tokenId] = dna;
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IERC721DnaMintable).interfaceId || super.supportsInterface(interfaceId);
    }
}
