...
import { ADDRESS_1 } from "@owlprotocol/web3-models";
import { ERC165CRUDActions, InferInterfaceAction, ContractCRUDActions, fetchERC165 } from "@owlprotocol/web3-actions";
import { ContractDexie, ERC165Dexie } from "@owlprotocol/web3-dexie";
import { NetworkSelectors } from "@owlprotocol/web3-redux-orm";
import { ContractSagaHelpers } from "../contracts.js";

export function* inferIERC165(networkId: string, address: string): Generator<any, boolean> {
    //IERC165
    try {
        const supported = yield* ContractSagaHelpers.IERC165.callSaga.supportsInterface({
            networkId,
            to: address,
...
export function* inferIERC20(networkId: string, address: string): Generator<any, string[] | undefined> {
    //IERC20
    try {
        const balanceOf = yield* ContractSagaHelpers.IERC20.callSaga.balanceOf({
            networkId,
            to: address,
            args: [ADDRESS_1],
        });
        if (balanceOf.returnValue) {
            ...
            try {
                const name = yield* ContractSagaHelpers.IERC20Metadata.callSaga.name({
                    networkId,
                    to: address,
                    args: [],
                });
...
export function* inferIERC721(networkId: string, address: string): Generator<any, string[] | undefined> {
    //IERC721
    try {
        const ownerOf = yield* ContractSagaHelpers.IERC721.callSaga.ownerOf({
            networkId,
            to: address,
            args: [1],
        });
        if (ownerOf.returnValue) {
            ...
            try {
                const tokenUri = yield* ContractSagaHelpers.IERC721Metadata.callSaga.tokenURI({
                    networkId,
                    to: address,
                    args: [1],
                });
...
export function* inferIERC1155(networkId: string, address: string): Generator<any, string[] | undefined> {
    //IERC1155
    try {
        const balanceOf = yield* ContractSagaHelpers.IERC1155.callSaga.balanceOf({
            networkId,
            to: address,
            args: [ADDRESS_1, 1],
        });
        if (balanceOf.returnValue) {
            ...
            try {
                const uri = yield* ContractSagaHelpers.IERC1155MetadataURI.callSaga.uri({
                    networkId,
                    to: address,
                    args: [1],
                });
...
export function* inferInterfaceSaga(action: InferInterfaceAction): Generator<any, string[]> {
    ...
    if (contract?.interfaceCheckedAt && Date.now() - contract.interfaceCheckedAt < interfaceCheckedAtMaxCacheAge) {
        const erc165 = yield* call(ERC165Dexie.where, { networkId, address });
        return erc165.map((c) => c.interfaceId); //ABI Known
    }

    yield put(ContractCRUDActions.actions.upsert({ networkId, address, interfaceCheckedAt: Date.now() }));
    const interfaceIds = yield* call(inferInterface, networkId, address);

    if (interfaceIds.length > 0) {
        ERC165CRUDActions.actions.updateBatched(
