...
import { inferInterfaceAction as inferInterfaceAction } from '../actions/index.js';
import { NetworkCRUD } from '../../network/crud.js';
import { ContractCRUD } from '../crud.js';
import { network1336 } from '../../network/data.js';
import { fetchSaga } from '../../network/sagas/fetch';
import sleep from '../../utils/sleep.js';

const networkId = network1336.networkId;
const web3 = network1336.web3!;
const address = ADDRESS_0;
const action = inferInterfaceAction({ networkId, address }, '');
...
describe(`${name}/sagas/getNonce.test.ts`, () => {
    describe('unit', () => {
        it('getNonce', () => {
            testSaga(getNonce, action)
                .next()
                .call(fetchSaga, NetworkCRUD.actions.fetch({ networkId }))
                .next({ networkId, web3 })
    ...
    describe('store', () => {
        ...
        it('getNonce', async () => {
            store.dispatch(inferInterfaceAction({ networkId, address }));
            await sleep(1000);
