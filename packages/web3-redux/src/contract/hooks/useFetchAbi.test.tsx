    ...
    describe("useFetchAbi", () => {
        it("(networkId, address, true)", async () => {
            const { result, waitForNextUpdate } = renderHook(
                () => useFetchAbi(networkId, address, true),
                {
                    wrapper,
                }
            );
            ...
            assert.deepEqual(
                result.all,
                [undefined, Contracts.Artifacts.IERC20.abi as any],
                "result.all"
            );
        });
        ...
        it("(networkId, address, false)", async () => {
            const { result } = renderHook(
                () => useFetchAbi(networkId, address, false),
                {
                    wrapper,
                }
            );
        ...
        it("(networkId, address, ifnull)", async () => {
            const { result, waitForNextUpdate } = renderHook(
                () => useFetchAbi(networkId, address, "ifnull"),
                {
                    wrapper,
                }
            );
