...
export function useERC721Owned(account: string, networkId?: string) {
    ...
    const tokenSingle = TransferEvents.map((e) => { return { networkId: e.networkId, address: e.address, tokenId: e.returnValues!.tokenId } })
    const tokens = uniq([...tokenSingle])

    const ownerOfFilters = tokens.map(({ networkId, address, tokenId }) => {
        return {
            networkId,
            address,
            methodName: 'ownerOf',
