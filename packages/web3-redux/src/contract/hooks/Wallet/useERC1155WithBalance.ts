...
import { flatMap, flatten, uniq } from 'lodash-es';
...
export function useERC1155WithBalance(account: string, networkId?: string, minBalance: BigNumberish = 0) {
    ...
    const tokenSingle = TransferSingleEvents.map((e) => { return { networkId: e.networkId, address: e.address, id: e.returnValues!.id } })
    ...
    const tokens = uniq([...tokenSingle, ...tokensMany])
