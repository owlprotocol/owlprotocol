...
describe(`${ContractName}/hooks/useGetBalance.test.tsx`, () => {
    let store: StoreType;
    let wrapper: any;
    let address: string;
    ...
    describe("useGetBalance", () => {
        it("(networkId, address, false)", async () => {
            const { result, waitForNextUpdate } = renderHook(
                () => useGetBalance(networkId, address, false),
                {
                    wrapper,
                }
            );
        ...
        it("(networkId, address, ifnull)", async () => {
            const { result, waitForNextUpdate } = renderHook(
                () => useGetBalance(networkId, address, "ifnull"),
                {
                    wrapper,
                }
            );
        ...
        it("(networkId, address, Transaction)", async () => {
            const { result, waitForNextUpdate } = renderHook(
                () => useGetBalance(networkId, address, "Transaction"),
                {
                    wrapper,
                }
            );
        ...
        it("(networkId, address, Block)", async () => {
            const { result, waitForNextUpdate } = renderHook(
                () => useGetBalance(networkId, address, "Block"),
                {
                    wrapper,
                }
            );
