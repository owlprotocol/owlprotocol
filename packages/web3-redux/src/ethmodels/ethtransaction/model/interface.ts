...
export type EthTransactionIndexInput =
    | EthTransactionId
    | { networkId: string }
...
export const EthTransactionIndex =
    "[networkId+hash], [networkId+blockNumber], [networkId+blockHash], [networkId+from+to], [networkId+to], [networkId+contractAddress]";

/** @internal */
export function validateId({ networkId, hash }: EthTransactionId): EthTransactionId {
    return { networkId, hash };
}

export function toPrimaryKey({ networkId, hash }: EthTransactionId): [string, string] {
    return [networkId, hash];
}

/** @internal */
export function validate(item: EthTransaction): EthTransaction {
