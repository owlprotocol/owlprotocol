...
export function validateId({ uuid }: EthSendId): EthSendId {
    return { uuid };
}

export function toPrimaryKey({ uuid }: EthSendId): [string] {
    return [uuid];
}

/** @internal */
export function validate<Args = any>(item: EthSendPartial<Args>): EthSend {
    //Interface
