...
export const HELLO_WORLD_BLOCK = {
    Data: Buffer.from(HELLO_WORLD).toString("base64"),
    Links: [],
};
...
export const NFT_1_BLOCK = {
    Data: Buffer.from(JSON.stringify(NFT_0)).toString("base64"),
    Links: [],
};
...
const ipfsMockStore: any = {
    [HELLO_WORLD_QMHASH]: {
        hash: HELLO_WORLD_QMHASH,
        content: HELLO_WORLD,
        pbNode: HELLO_WORLD_BLOCK,
    },
    [NFT_0_QMHASH]: { hash: NFT_0_QMHASH, content: NFT_0, pbNode: NFT_1_BLOCK },
    [NFT_COLLECTION_QMHASH]: {
        hash: NFT_COLLECTION_QMHASH,
        pbNode: NFT_COLLECTION_BLOCK,
    },
};
    ...
    return new Promise((resolve, reject) => {
        moxios.wait(async () => {
            ...
            if (request.config.url === "/api/v0/object/get") {
                ...
                const response = await request.respondWith({
                    status: 200,
                    response: pbNode,
                });
