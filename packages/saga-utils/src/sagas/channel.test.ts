...
describe("sagas/channel.test.ts", () => {
    ...
    it("channelTakeEveryPut(chan)", async () => {
    ...
    it("channelBufferBySplit(chan)", async () => {
        const action1 = { type: "START", val: 1 };
        const action2 = { type: "START", val: 2 };
        ...
        await expectSaga(mainSaga, chan).put([action1]).put([action2]).run({ timeout: 20, silenceTimeout: true });
    });
});
