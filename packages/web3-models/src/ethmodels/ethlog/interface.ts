...
export type EthLogIndexByContractInput = { networkId: string; address: string; eventFormatFull: string } & {
    topic0?: string;
    topic1?: string;
    topic2?: string;
    topic3?: string;
...
/** @internal */
export function validateIdEthLog({ networkId, blockNumber, logIndex }: EthLogId): EthLogId {
    return { networkId, blockNumber, logIndex };
}

export function toPrimaryKeyEthLog({ networkId, blockNumber, logIndex }: EthLogId): [string, number, number] {
    return [networkId, blockNumber, logIndex];
}
